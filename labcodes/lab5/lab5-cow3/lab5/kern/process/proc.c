...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    proc = alloc_proc();
    if (!proc) {
        goto fork_out;  // PCB 分配失败
    }
    
    // LAB5: 设置子进程的父进程为当前进程，确保当前进程的wait_state为0
    proc->parent = current;
    current->wait_state = 0;

    // 2. 分配并初始化内核栈
    if (setup_kstack(proc) != 0) {
        goto bad_fork_cleanup_proc;// 分配内核栈失败，需要释放 PCB
    }

    // 3. 根据clone_flags决定是复制还是共享内存管理系统
    if (copy_mm(clone_flags, proc) != 0) {
        goto bad_fork_cleanup_kstack;// 复制 mm 失败，需要释放内核栈 + PCB
    }
...
load_icode(unsigned char *binary, size_t size)
{
     ...
     */
    // 设置用户栈指针：USTACKTOP是用户栈顶地址
    tf->gpr.sp = USTACKTOP;
    
    // 设置程序入口点：ELF文件的入口地址
    tf->epc = elf->e_entry;
    
    // 设置sstatus寄存器：清除SPP位（设置为用户模式），设置SPIE位（允许中断）
    // SPP位为0表示返回用户模式，SPIE位为1表示在用户模式开启中断
