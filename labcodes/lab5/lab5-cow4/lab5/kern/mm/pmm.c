               ...
               bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
             ...
             * - map the same physical page into the child as read-only
             * - clear the write bit in the parent's pte
             * - increase the page refcount via page_insert for the child
             */
            struct Page *page = pte2page(*ptep);
            assert(page != NULL);
            // child mapping: clear write bit
            ...
            // insert the same page into child's pgdir (page_insert increments ref)
            if (page_insert(to, page, start, ro_perm) != 0)
            {
                return -E_NO_MEM;
            }
            // make parent's mapping read-only as well
