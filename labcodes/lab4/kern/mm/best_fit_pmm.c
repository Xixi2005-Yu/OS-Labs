...
*/
// LAB2 EXERCISE 1: 2312642 2312481 2312724
...
static void
best_fit_init_memmap(struct Page *base, size_t n) {//初始化空闲内存区间
    assert(n > 0);
    struct Page *p = base;
    for (; p != base + n; p ++) {
        assert(PageReserved(p));
        /*LAB2 EXERCISE 2: 2312642 2312481 2312724*/ 
...
static struct Page *
best_fit_alloc_pages(size_t n) {//分配函数（best-fit算法）
    assert(n > 0);
    if (n > nr_free) {
        return NULL;
    }
...
static void
best_fit_free_pages(struct Page *base, size_t n) {//释放与合并
    assert(n > 0);
    struct Page *p = base;
    for (; p != base + n; p ++) {
