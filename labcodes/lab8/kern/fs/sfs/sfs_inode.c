...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2312481 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    size_t bytes;
    uint32_t cur_blk = blkno;
    off_t cur_off = offset;

    while (cur_off < endpos) {
        blkoff = cur_off % SFS_BLKSIZE;
        if (blkoff != 0) {
            /*
             * （1）处理起始非对齐的部分块：
             * - 先通过 sfs_bmap_load_nolock 获取文件中该逻辑块对应的磁盘块号 (ino)
             * - 使用 sfs_buf_op（已指向 sfs_rbuf 或 sfs_wbuf）从块内偏移 blkoff 处读/写
             * - 读/写长度为到块末或到 endpos（如果后续没有完整块）
             * - 更新已处理字节数 alen、当前文件偏移 cur_off 以及块序号 cur_blk
             */
            ret = sfs_bmap_load_nolock(sfs, sin, cur_blk, &ino);
            if (ret != 0) goto out;

            bytes = SFS_BLKSIZE - blkoff;
            if (cur_off + bytes > endpos) bytes = endpos - cur_off;

            ret = sfs_buf_op(sfs, (char *)buf + alen, bytes, ino, blkoff);
            if (ret != 0) goto out;
        ...
        if (endpos - cur_off >= SFS_BLKSIZE) {
            /*
             * （2）处理整块对齐的完整块：
             * - 对每个完整块调用 sfs_bmap_load_nolock 获取磁盘块号 (ino)
             * - 使用 sfs_block_op（已指向 sfs_rblock 或 sfs_wblock）传输整块数据
             * - 更新 alen、cur_off、cur_blk（可在循环中一次处理多块）
             */
            ret = sfs_bmap_load_nolock(sfs, sin, cur_blk, &ino);
            if (ret != 0) goto out;

            ret = sfs_block_op(sfs, (char *)buf + alen, ino, 1);
            if (ret != 0) goto out;
        ...
        /*
         * （3）处理最后一个非对齐的尾部部分：
         * - 通过 sfs_bmap_load_nolock 获取对应磁盘块号 (ino)
         * - 使用 sfs_buf_op 从块起始处读/写 bytes 个字节（bytes = endpos - cur_off）
         * - 更新 alen 和 cur_off，然后结束循环
         */
        ret = sfs_bmap_load_nolock(sfs, sin, cur_blk, &ino);
        if (ret != 0) goto out;

        bytes = endpos - cur_off;
        ret = sfs_buf_op(sfs, (char *)buf + alen, bytes, ino, 0);
        if (ret != 0) goto out;
