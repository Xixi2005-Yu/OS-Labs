...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2312481 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
        ...
        // LAB8 add:
        proc->filesp = NULL;          // 初始化文件表指针为空，后续在 do_fork 或创建 initproc 时分配
...
void proc_run(struct proc_struct *proc)
{
    ...
    // LAB4:填写你在lab4中实现的代码
        ...
        */
    //LAB8 2312481 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
       ...
       */
        ...
        local_intr_save(intr_flag);
        {
            ...
            // Switch page table to target proc
            if (proc->mm != NULL)
            {
                lsatp(proc->pgdir);
            }
            ...
            // LAB8(update LAB4 steps)
            //刷新TLB，保证切换到新进程的页表后不会使用旧的TLB缓存
            flush_tlb();  
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2312481  HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
    ...
    { // for LAB8
        goto bad_fork_cleanup_fs;  // 回收文件结构相关资源
    }
    //    4. call copy_thread to setup tf & context in proc_struct
...
{
    /* LAB8:EXERCISE2 2312481  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
     ...
     */
    int ret = -E_NO_MEM;
    struct mm_struct *mm = NULL;       // 当前进程的内存管理结构
    ...
    //(3) read ELF file header from fd (at offset 0)
    ...
    //(3.2) read program headers from e_phoff
    for (i = 0; i < elf.e_phnum; i++) {
        ...
        // (3.4) alloc pages for TEXT/DATA and copy from file (use file offset ph.p_offset)
        ...
        while (start < file_end) {
            ...
            if (file_end < la + PGSIZE)
                size = file_end - start;

            off_t file_off = ph.p_offset + (start - ph.p_va);
        ...
        // (3.5) alloc pages for BSS and memset 0
    ...
    uint32_t stack_top = USTACKTOP;
    vm_flags = VM_READ | VM_WRITE | VM_STACK;
    if ((ret = mm_map(mm, stack_top - USTACKSIZE, USTACKSIZE, vm_flags, NULL)) != 0)
        goto bad_cleanup_mmap;

    for (i = 0; i < 4; i++) // 分配4页用户栈
        assert(pgdir_alloc_page(mm->pgdir, stack_top - (i + 1) * PGSIZE, PTE_USER) != NULL);

    /* (5) set current process's mm and flush CR3 so user memory is accessible */
    mm_count_inc(mm);
    current->mm = mm;
    current->pgdir = PADDR(mm->pgdir);
    lsatp(PADDR(mm->pgdir)); // 刷新页表寄存器

    /* (6) setup argc/argv on user stack: copy strings into user stack and build argv */
    ...
    for (i = argc - 1; i >= 0; i--) {
        ...
        uargv[i] = sp;
        if (i == 0) break; /* avoid unsigned wrap of i */
    }
    /* align pointer */
    ...
    /* argv[argc] = NULL */
    uintptr_t zero = 0;
    if (!copy_to_user(mm, (void *)(sp + argc * sizeof(uintptr_t)), &zero, sizeof(uintptr_t))) {
        ret = -E_INVAL;
        goto bad_cleanup_mmap;
    }
    ...
    memset(tf, 0, sizeof(struct trapframe));
    tf->status = (sstatus & ~SSTATUS_SPP) | SSTATUS_SPIE; // 设置用户态返回
    tf->gpr.sp = sp;        // 指向 argv 指针数组
    tf->gpr.a0 = argc;      // argc
    tf->gpr.a1 = sp;        // argv
